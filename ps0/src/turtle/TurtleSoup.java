...
public class TurtleSoup {
    ...
    public static double calculateRegularPolygonAngle(int sides) {
        if (sides > 2) {
            ...
            System.out.println("Error message: sides number is less than 2!");
            return 0.0;
        }
    ...
    public static double calculateHeadingToPoint(double currentHeading, int currentX, int currentY, int targetX, int targetY) {
        double angle;
        angle = Math.PI/2 - Math.atan2(targetY - currentY, targetX - currentX)  - currentHeading;
        if (angle < 0) {
    ...
    public static List<Double> calculateHeadings(List<Integer> xCoords, List<Integer> yCoords) {
        ...
        if (xCoords.size() == yCoords.size()) {
            for (int i = 0; i < xCoords.size(); i++) {
             double adjustment =   calculateHeadingToPoint(0, xCoords.get(i), yCoords.get(i), xCoords.get(i+1), yCoords.get(i+1));
             adjustments.add(adjustment);
            }
            return adjustments;
            
        } else {
            System.out.println("The lists length don't meet!");
